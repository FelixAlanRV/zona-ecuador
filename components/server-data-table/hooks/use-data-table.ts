...
interface UseDataTableProps<TData>
  ...
  /**
   * A unique key to identify the table and persist its column visibility state
   * in localStorage. If not provided, visibility will not be persisted.
   */
  storageKey?: string;
...
export function useDataTable<TData>({
  ...
  >(() => {
   ...
   // 1. Llamamos a los hooks de forma incondicional, como dicta React.
  // Hook para el estado local (no persistente)
  const [nonPersistentVisibility, setNonPersistentVisibility] =
    React.useState<VisibilityState>(initialState?.columnVisibility ?? {});

  // Hook para el estado global (persistente)
  const { tablesVisibility, setTableVisibility } = useTableSettingsStore();
   // 2. Decidimos qué estado y qué función de actualización usar.
  // Usamos useMemo para que estas variables solo se recalculen si sus dependencias cambian.
  const { columnVisibility, onColumnVisibilityChange } = React.useMemo(() => {
    if (storageKey) {
      // MODO PERSISTENTE: Usamos Zustand
      const visibilityState = tablesVisibility[storageKey] ?? initialState?.columnVisibility ?? {};
      
      const setter = (updater: Updater<VisibilityState>) => {
        const newVisibility =
          typeof updater === "function" ? updater(visibilityState) : updater;
        setTableVisibility(storageKey, newVisibility);
      };

      return {
        columnVisibility: visibilityState,
        onColumnVisibilityChange: setter,
      };
    } else {
      // MODO NO PERSISTENTE: Usamos useState local
      return {
        columnVisibility: nonPersistentVisibility,
        onColumnVisibilityChange: setNonPersistentVisibility,
      };
    }
  }, [
    storageKey,
    ...
    nonPersistentVisibility,
    // No necesitamos setNonPersistentVisibility aquí porque React garantiza que es estable.
  ]);
